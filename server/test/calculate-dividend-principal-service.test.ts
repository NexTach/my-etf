...
describe("CalculateDividendPrincipalService", () => {
  describe("given completed investments and a partial completed withdrawal", () => {
    describe("when dividend principal is calculated for the withdrawal month", () => {
      it("then subtracts the withdrawal FIFO before returning eligible principal", () => {
        const result = new CalculateDividendPrincipalService().execute({
          ...
          investments: [
            { id: "old", userId: "user-1", amountKrw: 100_000, completedAt: "2026-06-10T00:00:00.000Z" },
            { id: "new", userId: "user-1", amountKrw: 50_000, completedAt: "2026-07-10T00:00:00.000Z" }
          ],
          withdrawals: [
            { id: "withdrawal-1", userId: "user-1", amountKrw: 120_000, completedAt: "2026-08-05T00:00:00.000Z" }
          ]
        });
  ...
  describe("given a withdrawal completed after the selected month", () => {
    describe("when an earlier month is calculated", () => {
      it("then does not reduce that earlier month retroactively", () => {
        const result = new CalculateDividendPrincipalService().execute({
          ...
          investments: [
            { id: "investment-1", userId: "user-1", amountKrw: 100_000, completedAt: "2026-06-10T00:00:00.000Z" }
          ],
          withdrawals: [
            { id: "withdrawal-1", userId: "user-1", amountKrw: 100_000, completedAt: "2026-08-05T00:00:00.000Z" }
          ]
        });
